  ...
  return (
    ...
    >
      <div className="relative h-48 overflow-hidden">
        ...
        <Badge className="absolute top-3 left-3 bg-primary text-primary-foreground">{item.type}</Badge>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-1 text-muted-foreground text-sm mb-3">
          <MapPin className="w-3 h-3" /><span>{item.distance} from campus</span>
        </div>
        <div className="flex gap-2 mb-3">
          {item.amenities.slice(0, 3).map((amenity) => (
            <div key={amenity} className="p-1.5 bg-secondary rounded-lg" title={amenity}>{getAmenityIcon(amenity)}</div>
          ))}
        </div>
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-20 pb-8">
        <div className="relative h-56 md:h-72 overflow-hidden">
          <img src="https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=1200" alt="Accommodation Banner" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-br from-violet-900/80 via-purple-800/60 to-indigo-700/50" />
          <div className="absolute inset-0 flex items-end pb-8">
            <div className="container mx-auto px-4">
              ...
              <h1 className="text-3xl md:text-5xl font-bold text-white tracking-tight">Accommodation</h1>
              <p className="text-white/80 mt-2 text-lg">Find your perfect home away from home</p>
            </div>
          </div>
        </div>
